...
def pull_db(columns=None, table_name="reviews"): # columns is expecting a collection of strings
    ...
    if (columns == None) and (table_name == "reviews"): # column as none guard
        columns = [
                "review_text",
                "review_date",
                "review_id",
    ...
    elif (columns == None) and (table_name == "lexicons"): # same like above but with the lexicons table
        columns = [
            "slang",
            "formal"
        ]
...
def push_db(df, columns=None, table_name=None):
    ...
    if (table_name == None) and (columns == None): # none guard thing
    ...
    if columns:
        for i in columns: # no column in dataframe guard
            if i not in df.columns:
                print(f"Column {i} not found in dataframe provided. Check the formatting and if the column is correct.")
                error_status = f"Column {i} not found in dataframe provided. Check the formatting and if the column is correct."
                return error_status
    ...
    try: # the if and elif are for existing column in our db already, but we made it so that it can accomodate other columns and tables in our database as well like we did above (should there be any)
        if (table_name == "reviews") and (columns == None): # if table name was given as "review" but no column provided, it defaults to the ones available in our database
            numpy_df = df[[ 
                "review_text",
                "review_date",
                "review_id",
        ...
        elif (table_name == "lexicons") and (columns == None): # same like above but with the lexicons table
            numpy_df = df[[
                "slang",
                "formal"
            ]].to_numpy()
            c.executemany("""
                ...
                VALUES (?, ?);
            """, numpy_df)
        elif (table_name != None) and (columns != None): # if both table name and columns are provided, it will use the columns to insert data into the table
        ...
        elif (table_name != None) and (columns == None): # if a table name was provided but not columns, it will assume that all columns from the dataframe will be pushed into the table in the database
